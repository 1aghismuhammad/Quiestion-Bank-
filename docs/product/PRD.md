...
- Status: Phase 0 through Phase 5 are `COMPLETE`. Phase 5 Question Bank is MCQ-only. Phase 5.7 is `IN PROGRESS`. Phase 5.7A through Phase 5.7B3 are `COMPLETE`. Phase 5.7C+D remains implemented and pending final manual QA. Phase 5.7E is `COMPLETE` (Advanced MCQ, Pro gating, mixed difficulty, deterministic shuffle). Phase 5.7F (True/False and Essay generation) is `NOT STARTED`. Phase 5.7G (Run-to-Question-Bank import, review/edit flow, question DOCX, and final hardening) is `NOT STARTED`. Phase 6 Admin Dashboard remains `PLANNED`.
...
- FR-BP-02: Bentuk Simple: 1–5 baris MCQ, satu tingkat kesulitan, total 1–10. Bentuk Advanced: 1–5 baris MCQ, mixed difficulty diizinkan, 1–10 per baris, total 1–30. Setiap baris wajib punya mapping konteks bounded yang tervalidasi dari elemen/chunk Profil. Confirm dan AI fill menolak Profil missing/stale/foreign/failed/queued/processing tanpa menulis kuota. First-N/full-book fallback dilarang. Mode confirmed immutable. Historical Blueprint tanpa kolom mode resolve ke Simple.
- FR-BP-03: AI fill memakai provider Blueprint tersendiri. Simple memakai `blueprint-fill-v1`. Advanced memakai `blueprint-fill-v2` dan target 1–30 yang disimpan sebagai `ai_fill_requested_total` bersama token workflow baru. Paling banyak tiga **accepted queue events** per user per jam bergulir, termasuk retry draf yang sama. Satu fill in-flight per Material, tetap `draft`, tidak auto-confirm, dan tidak menulis `ai_usage_logs`. Offset provider bersifat relatif terhadap excerpt `context_ref`; server yang mengonversi ke offset kanonis.
...
- FR-RUN-01: Owner dapat memulai Simple Generation Run dari Blueprint confirmed yang masih current plus Profil ready yang cocok. Total 1–10, MCQ, satu difficulty, tanpa shuffle. Advanced Run mensyaratkan Pro aktif, MCQ, 1–30 soal, mixed difficulty diizinkan, dan kualifikasi: total 11–30 lolos karena skala; total 1–10 membutuhkan mixed difficulty atau shuffle soal/opsi. Mode Run di-snapshot dari Blueprint persisted. Posted mode/shuffle untuk Simple `true` ditolak sebelum side effect.
...
- FR-RUN-03: Question Bank import, editor soal generated, dan question DOCX adalah Phase 5.7G, bukan 5.7E. Owner dapat melihat soal completed secara read-only, escaped, bernomor lintas child, dengan Kunci yang sudah di-remap jika opsi diacak. Shuffle deterministik SHA-256 dari identitas Run; `result_json` kanonis tidak diubah.
...
- Quota diperiksa sebelum generation (definisi limit Phase 3.5; reservation/charge/release Phase 4.1+4.2; Gemini MCQ job Phase 4.3+4.4; owner UI Phase 4.5; SUM ledger, Simple Run, dan Advanced MCQ Run Phase 5.7D+E).
- Gemini menghasilkan MCQ terstruktur yang divalidasi server-side (4.3+4.4). True/false dan essay belum dijalankan provider. Owner dapat mengonfigurasi generasi, memantau queued/processing, melihat pratinjau completed, dan retry failed. Owner dapat membuat kisi-kisi Simple atau Advanced, mengonfirmasi, mengisi dengan AI (0 credit), mengunduh DOCX confirmed, dan menjalankan Generation Run (`ceil(n/10)` credit; Simple 1–10 tanpa shuffle; Advanced Pro 1–30 dengan mixed difficulty dan pengacakan deterministik). True/False dan Essay belum (Phase 5.7F). Run-to-Question-Bank import, alur review/edit, dan question DOCX belum (Phase 5.7G).
