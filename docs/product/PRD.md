...
- FR-BP-06: K2A menambahkan pondasi backend untuk import Blueprint dari DOCX (tanpa UI publik, tanpa pemanggilan Gemini, tanpa Draft Blueprint, tanpa kredit generasi). Duplikat file yang aktif ditolak di application level. Menyimpan provenance dari versi Profil saat import dibuat. K2A.1 mengeraskan job (`ShouldBeUnique` + `WithoutOverlapping` per import, `timeout=60`, `failOnTimeout`, backoff `[10,30,60]`), mengisi `queued_at` pada enqueue attempt, men-terminalisasi kegagalan dispatch menjadi FAILED yang disanitasi, dan membersihkan source secara best-effort. Operasi ekstraksi tetap idempotent pada EXTRACTED/FAILED; PROCESSING tetap reclaimable untuk retry operasional.
