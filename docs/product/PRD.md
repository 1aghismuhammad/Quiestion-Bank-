...
- Status: Phase 0 through Phase 5 are `COMPLETE`. Phase 5.7G is `COMPLETE` (Run-to-Question-Bank import, typed edit/publish, question DOCX). K2A (Question Blueprint DOCX Import Foundation) is `COMPLETE`. K2A.1 (Blueprint Import Runtime Hardening) is `IMPLEMENTED — QA PENDING`. K2B-0 is `PASS` (evidence favored Decision B). K2B.1 (Structured DOCX Persistence) is `IMPLEMENTED — QA PASS`. K2B.2 (AI Interpretation Foundation) is `COMPLETE + APPROVED`. K2B.3 (Owner Review Surface) is `IMPLEMENTED — SOURCE REVIEW PASS — MANUAL QA DEFERRED` (manual/browser QA deferred to Final Phase K). K2C (Material Grounding) is `IMPLEMENTED — SOURCE REVIEW PASS — MYSQL QA PASS — PROVIDER SMOKE PASS — MANUAL QA DEFERRED` (technical/source/MySQL/provider gates closed; manual/browser + shared-cache/multi-worker Final Phase K QA remain deferred; not `COMPLETE + APPROVED`). K2B as a whole is not complete. K2D (Draft Blueprint from DOCX import and the Blueprint creation hub) is `IMPLEMENTED` (planning, implementation, source review, automated QA, MySQL concurrency QA, database QA, UI creation hub, Indonesian localization, and staging review PASS). K2D MANUAL / BROWSER QA: DEFERRED BY OWNER. K2D `COMPLETE + APPROVED`: NO. Phase 6 Admin Dashboard remains `PLANNED`.
...
4. Sistem memvalidasi materi, konfigurasi, dan quota, lalu mereservasi credit. Legacy Start mereservasi satu credit per Generation. Generation Run mereservasi `ceil(n/10)` credit pada subjek Run (1–10 = 1, 11–20 = 2, 21–30 = 3).
...
- FR-BP-02: Bentuk Simple: 1–5 baris, satu `question_type` bersama (`multiple_choice`, `true_false`, atau `essay`), satu tingkat kesulitan, total 1–10, tanpa shuffle. Bentuk Advanced: 1–5 baris, tipe dan difficulty boleh berbeda, 1–10 per baris, total 1–30. Setiap baris wajib punya mapping konteks bounded yang tervalidasi dari elemen/chunk Profil. Confirm dan AI fill menolak Profil missing/stale/foreign/failed/queued/processing tanpa menulis kuota. First-N/full-book fallback dilarang. Mode confirmed immutable. Historical Blueprint tanpa kolom mode resolve ke Simple.
...
- FR-BP-06: K2A menambahkan pondasi backend untuk import Blueprint dari DOCX (pada K2A: tanpa UI publik, tanpa pemanggilan Gemini, tanpa Draft Blueprint, tanpa kredit generasi; unggah owner dan draf impor ada pada K2D). Duplikat file yang aktif ditolak di application level. Menyimpan provenance dari versi Profil saat import dibuat. K2A.1 mengeraskan job (`ShouldBeUnique` + `WithoutOverlapping` per import, `timeout=60`, `failOnTimeout`, backoff `[10,30,60]`), mengisi `queued_at` pada enqueue attempt, men-terminalisasi kegagalan dispatch menjadi FAILED yang disanitasi, dan membersihkan source secara best-effort. Operasi ekstraksi tetap idempotent pada EXTRACTED/FAILED; PROCESSING tetap reclaimable untuk retry operasional.
- FR-BP-07: K2B.1 menambahkan representasi struktural import-specific (`structured_document`, `structure_schema_version=blueprint-import-structure-v1`) yang dipersist bersama `extracted_text` sebelum cleanup source. `extracted_text` tetap dihasilkan oleh Material `DocxExtractor` yang tidak diubah (kompatibilitas K2A); K2B-0 membuktikan teks polos tidak cukup sebagai satu-satunya input interpretasi. Ekstraksi baru menjadi EXTRACTED hanya jika struktur dan teks keduanya sukses. Baris EXTRACTED pra-K2B.1 boleh memiliki kolom struktur NULL; tidak ada backfill dan tidak ada pelemahan duplikat same-hash. K2B.1 tidak memanggil Gemini, tidak membuat kandidat/review UI/Draft Blueprint/QuestionBlueprintRow, tidak melakukan grounding Material, dan tidak menagih kredit generasi. Batas struktur import (`max_blocks` 500, `max_rows` 500, dll.) terpisah dari Draft Blueprint `max_rows=5`.
- FR-BP-08: K2B.2 menambahkan fondasi interpretasi AI terpisah setelah EXTRACTED. Input otoritatif adalah `structured_document` + `structure_schema_version` (bukan `extracted_text`). Job `InterpretQuestionBlueprintImport` berjalan di `database-generation` / `material-intelligence` (`timeout=270`, `tries=3`, `backoff=[5,15]`, `failOnTimeout=false`) dengan `WithoutOverlapping` per import (`blueprint-import-interpretation:{importId}`, `releaseAfter=60`, `expireAfter=330`) dan **tanpa** `ShouldBeUnique`. Status interpretasi terpisah dari ekstraksi: `queued|processing|review_ready|failed` (NULL = belum diinisialisasi / historis / tidak eligible). Prompt immutable `blueprint-import-interpret-v1`; konten DOCX adalah DATA tidak tepercaya. Provider hanya mengembalikan source refs; server (`BlueprintImportInterpretationResultBuilder`) memvalidasi fail-closed dan meresolusi raw values dari struktur. Schema provider membedakan paragraph ref (`kind`+`block_ordinal`) vs cell ref (`kind`+`block_ordinal`+`table_index`+`row_index`+`cell_index`). Bound serialisasi 262144 byte; kandidat maks 100 (101 menolak seluruh respons). Canonical Blueprint fields tetap null. Zero generation credit; tidak menulis `ai_usage_logs`; tidak membuat QuestionBlueprint/Row, Generation Run, atau QuestionSet. Grounding = K2C; Draft publik = K2D.
- FR-BP-09: K2B.3 menambahkan Owner Review Surface read-only untuk hasil interpretasi import. Owner dapat melihat ringkasan impor terbaru di Material Detail, membuka riwayat impor (15/halaman), membuka tinjauan `REVIEW_READY`, memeriksa nilai mentah kandidat, peringatan/unresolved, dan lokasi provenance sumber (Option A; tanpa rekonstruksi teks sumber), mengamati status queued/processing lewat polling minimal, serta mencoba ulang interpretasi **hanya** saat `FAILED`. Tinjauan K2B.3 bukan approval dan bukan Final Blueprint; tidak melakukan grounding Material (K2C), kanonikalisasi/Draft (K2D), generasi soal, atau konsumsi kredit. Nested ownership wajib (`material` milik user, `import` milik user, `import.material_id` cocok); cross-material tampering → 404. Hasil persist yang malformed fail-closed. Tidak ada migrasi/skema baru. Manual/browser QA ditunda ke Final Phase K; status milestone: `IMPLEMENTED — SOURCE REVIEW PASS — MANUAL QA DEFERRED`.
- FR-BP-10: K2C menambahkan Material Grounding setelah interpretasi `REVIEW_READY`. Otoritas: DOCX = planning input; interpretation = semantic reading; pinned Material Profile = factual authority; `grounding_result` = evidence relationship. Draft Blueprint = K2D. Ground hanya `raw_objective`/`raw_topic`/`raw_material`/`raw_indicator`; klaim kosong di-assign server `not_applicable` dan tidak dikirim ke provider. Eligibility memakai exact `import.profile_version_id` (READY + ownership) plus fingerprint triple import≡Profile≡live Material; pin tidak diganti Profile READY terbaru. Trigger eksplisit `QueueQuestionBlueprintImportGrounding` saja (tidak auto dari K2B.2). Job `GroundQuestionBlueprintImport` (`database-generation` / `material-intelligence`, `timeout=270`, `tries=3`, `backoff=[5,15]`, `failOnTimeout=false`) dengan `WithoutOverlapping` `blueprint-import-grounding:{importId}` (`releaseAfter=60`, `expireAfter=330`) tanpa `ShouldBeUnique`. Provider (`QuestionBlueprintImportGroundingProvider` / Gemini) memilih `profile_element_ids` saja; server membangun evidence. Prompt `blueprint-import-ground-v1`; hasil schema `blueprint-import-grounding-result-v1` termasuk `interpretation_result_sha256` dari raw bytes. Katalog EXTRACTED max 200; request max 262144 byte; max 4 refs/klaim; sparse candidate contract. Empty/taxonomy: tanpa Gemini, `ready` + kandidat kosong + `document_rollup=not_applicable`. Zero credit pada grounding itu sendiri; pembuatan draf dari impor adalah K2D (FR-BP-11). Kontrol owner **Cocokkan dengan Materi** ada di halaman tinjauan impor. Migrasi `2026_09_23_000001_add_grounding_columns_to_question_blueprint_imports_table` sudah diterapkan pada persistent MySQL `ai_question_bank` (batch 6); `db:integrity-audit` PASS. Status current: `IMPLEMENTED — SOURCE REVIEW PASS — MYSQL QA PASS — PROVIDER SMOKE PASS — MANUAL QA DEFERRED` (bukan `COMPLETE + APPROVED`). Manual/browser QA dan shared-cache/multi-worker runtime proof tetap Final Phase K.
- FR-BP-11: K2D menambahkan tiga cara membuat kisi-kisi pada halaman Blueprint `/materials/{material}/blueprints`: **Buat Manual**, **Buat dengan AI**, dan **Unggah Kisi-kisi DOCX**. Unggah DOCX tidak lagi menjadi kontrol unggah utama di Material Detail. Material Detail tetap menampilkan ringkasan/riwayat impor dan tautan ke halaman kisi-kisi. Alur unggah: impor DOCX, ekstraksi, interpretasi, tinjauan owner, pencocokan eksplisit dengan materi, pemilihan kandidat yang layak, pengisian isian kanonis oleh owner, lalu draf kisi-kisi pada editor yang sudah ada. Konfirmasi tetap eksplisit, kemudian alur generasi yang sudah ada. Satu impor menghasilkan paling banyak satu Blueprint; konversi ulang membuka Blueprint yang sama. Blueprint biasa tetap memakai Profil READY terbaru yang cocok. Blueprint yang lahir dari impor tetap sah terhadap Profil yang di-pin pada impor itu selama Profil itu READY, pemilik dan materi cocok, dan fingerprint masih cocok; Profil READY yang lebih baru saja tidak membatalkannya. Clone impor yang pin-nya bukan Profil terbaru ditolak sebelum ditulis. AI fill dan unduh DOCX confirmed tidak berubah. Advanced tetap mensyaratkan Pro. Status: `IMPLEMENTED`. K2D MANUAL / BROWSER QA: DEFERRED BY OWNER. `COMPLETE + APPROVED`: NO.

- FR-RUN-01: Owner dapat memulai Simple Generation Run dari Blueprint confirmed biasa plus Profil READY terbaru yang cocok. Blueprint confirmed yang lahir dari impor memakai Profil yang di-pin impor selama pin itu masih READY dan fingerprint cocok. Total 1–10, satu tipe, satu difficulty, tanpa shuffle. Advanced Run mensyaratkan Pro aktif, 1–30 soal, mixed type/difficulty diizinkan, dan kualifikasi: total 11–30 lolos karena skala; total 1–10 membutuhkan mixed difficulty, mixed type, atau shuffle soal/opsi. `shuffle_options` ditolak jika tidak ada baris MCQ. Mode Run di-snapshot dari Blueprint persisted. Posted mode/shuffle untuk Simple `true` ditolak sebelum side effect.
...
- FR-AI-04: User memilih satu question type per generation legacy. Runtime Phase 4.3+4.4 mengeksekusi **multiple choice** saja (empat opsi A–D, satu jawaban, explanation wajib). Generation Run Phase 5.7F mengeksekusi MCQ (`mcq-v4` untuk Run baru, `mcq-v3` default legacy), True/False (`true-false-v2` untuk Run baru, `true-false-v1` default legacy; `correct_answer` JSON boolean), dan Essay (`essay-v2` untuk Run baru, `essay-v1` default legacy; `model_answer` + `rubric` teks). Legacy Start tetap menolak non-MCQ.
...
- FR-QB-01: Question Bank adalah Phase 5. Generation Phase 4 tidak membuat atau memerlukan draft `question_sets`. Owner mengimpor generation **completed** MCQ menjadi Question Set `draft` (satu Generation → paling banyak satu Question Set, `UNIQUE(generation_id)`). Phase 5.7G menambahkan import Run **completed** typed (MCQ, True/False, Essay) melalui `POST /generation-runs/{id}/question-sets` (satu Run → paling banyak satu Question Set, `UNIQUE(generation_run_id)`). `generation_id` dan `generation_run_id` mutually exclusive (MySQL CHECK `qs_source_exclusive_chk`; SQLite `QuestionSetSourceExclusive`). Persistensi adalah snapshot terpisah; `result_json` generasi/Run tidak dipindah atau dihapus. Pembuatan manual tetap later.
- FR-QB-02: User dapat meninjau dan mengedit pertanyaan pada Question Set `draft` sebelum publish. Phase 5 MVP: MCQ (judul, teks, opsi A–D, jawaban benar, penjelasan). Phase 5.7G: MCQ, True/False (Benar/Salah), dan Essay (`model_answer`, `rubric`) pada set hasil Run import. Satu halaman, satu simpan, atomik. Published read-only. Edit/publish tidak memanggil Gemini dan tidak menagih kuota.
- FR-QB-08: Owner dapat mengunduh DOCX soal published: versi siswa (`Soal-[title].docx`) dan versi guru/kunci (`Soal-Kunci-[title].docx`). Hanya published. PhpWord, try/finally temp cleanup, `deleteFileAfterSend`. Tidak ada token, secret, prompt, atau metadata provider dalam export.
