...
- FR-AI-04: User memilih satu question type per generation legacy. Runtime Phase 4.3+4.4 mengeksekusi **multiple choice** saja (empat opsi A–D, satu jawaban, explanation wajib). Generation Run Phase 5.7F mengeksekusi MCQ (`mcq-v3`), True/False (`true-false-v1`; `correct_answer` JSON boolean), dan Essay (`essay-v1`; `model_answer` + `rubric` teks). Legacy Start tetap menolak non-MCQ.
