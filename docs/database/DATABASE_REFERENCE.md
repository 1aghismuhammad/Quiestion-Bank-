...
K2C eligibility memakai pinned `profile_version_id` + fingerprint triple (`material_content_hash`, `material_file_hash`, `extractor_implementation`) terhadap Profile dan live Material. Katalog EXTRACTED max 200; request serialized max 262144 byte; max 4 refs per factual claim. Job `GroundQuestionBlueprintImport` mirror kontrak antrian interpretasi dengan overlap key `blueprint-import-grounding:{importId}`. Trigger eksplisit saja. Grounding sendiri tidak membuat Draft dan tidak memotong kredit. Shared-cache/multi-worker runtime proof tetap Final Phase K (SQLite in-memory suite bukan bukti MySQL concurrency).

K2D menambahkan satu kolom nullable tanpa default dan tanpa backfill (migrasi `2026_09_23_120000_add_created_blueprint_id_to_question_blueprint_imports_table`; Database QA PASS pada persistent `ai_question_bank`):

- `created_blueprint_id` BIGINT UNSIGNED NULL — FK ke `question_blueprints.blueprint_id`, `ON DELETE RESTRICT`, index pendukung biasa non-unique (`qbp_import_created_blueprint_fk`, `Non_unique = 1`). Bukan UNIQUE. NULL berarti impor belum dikonversi, termasuk impor historis saat migrasi diterapkan. Paling banyak satu Blueprint kanonis per impor dijaga di aplikasi: lock baris impor, cek pointer, satu transaksi, dan update bersyarat `whereNull`. Konversi ulang mengembalikan Blueprint yang sudah ada. Cardinality: banyak impor boleh menunjuk Blueprint yang sama secara skema; produk menolak lebih dari satu impor per Blueprint lewat resolver, bukan lewat unique index.
