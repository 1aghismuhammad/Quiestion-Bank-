...
- Phase 5.7C+D Blueprint dan Simple Generation Run UI menggunakan Blade/controller dan vanilla JS polling bounded, tanpa Livewire/React/Vue/websockets.
...
13. Credit direservasi pada Start. Legacy Start: satu Generation = satu reservation `credits=1`. Simple Run: satu reservation pada Run dengan `credits = ceil(n/10)` (1–10 = 1). Child Run tidak punya baris usage. Generation tidak memerlukan draft `question_sets`.
