...
- Google Gemini melalui queue untuk generation, analisis profil materi, AI fill kisi-kisi, interpretasi import Blueprint (K2B.2), dan pencocokan materi impor (K2C). K2D mengonversi tinjauan impor menjadi draf Blueprint secara sinkron tanpa kredit generasi. Unggah DOCX kisi-kisi ada di halaman kisi-kisi.
...
9k. K2B.3 Owner Review Surface (`IMPLEMENTED — SOURCE REVIEW PASS — MANUAL QA DEFERRED`): alur owner `DOCX import → extraction → AI interpretation → REVIEW_READY → Owner Review`. Material Detail menampilkan ringkasan impor terbaru + tautan riwayat. Riwayat impor read-only (15/halaman, newest first). Show review menampilkan kandidat raw (20/halaman in-memory, urutan tersimpan), warnings/unresolved, provenance lokasi saja (Option A; tanpa walk `structured_document`). Nested ownership: material milik user, import milik user, `import.material_id` cocok; cross-material → 404. Status JSON allowlisted + `Cache-Control: no-store`; polling hanya saat in-flight, reload saat terminal. HTTP retry hanya `EXTRACTED` + `FAILED` (UI hide bukan otoritas). Hasil persist malformed / kombinasi state mustahil fail-closed. Zero side effect generasi: tidak membuat QuestionBlueprint/Row, Generation Run, QuestionSet, atau charge `ai_usage_logs`. Manual/browser QA K2B.3 tetap ditunda ke Final Phase K. Pencocokan materi dan pembuatan draf dari impor ada pada 9l dan 9m. K2B.3 sendiri tidak mengonfirmasi Blueprint.
9l. K2C Material Grounding (`IMPLEMENTED — SOURCE REVIEW PASS — MYSQL QA PASS — PROVIDER SMOKE PASS — MANUAL QA DEFERRED`): setelah `REVIEW_READY`, Action eksplisit `QueueQuestionBlueprintImportGrounding` mengantrekan `GroundQuestionBlueprintImport` (`database-generation` / `material-intelligence`, `timeout=270`, `tries=3`, `backoff=[5,15]`, `failOnTimeout=false`) dengan `WithoutOverlapping` `blueprint-import-grounding:{importId}` (`releaseAfter=60`, `expireAfter=330`) tanpa `ShouldBeUnique`. Tidak auto-enqueue dari K2B.2. Eligibility: EXTRACTED + REVIEW_READY + interpretation-result-v1 valid + pinned READY Profile ownership + fingerprint import≡Profile≡live Material. Provider (`blueprint-import-ground-v1`) menerima sparse factual claims + katalog EXTRACTED (max 200) dan mengembalikan `status` + `profile_element_ids`; server membangun evidence dan rollup ke schema `blueprint-import-grounding-result-v1` (hash raw `interpretation_result`). Empty/taxonomy: tanpa Gemini, `ready` + kandidat kosong + `document_rollup=not_applicable`. Status grounding: `queued → processing → ready|failed`; transient `PROCESSING→QUEUED` same cycle; FAILED retry token lebih baru. Grounding sendiri zero credit dan tidak membuat Draft. Kontrol owner **Cocokkan dengan Materi** ada di halaman tinjauan (K2D). Migrasi `2026_09_23_000001_add_grounding_columns_to_question_blueprint_imports_table` sudah diterapkan pada persistent MySQL `ai_question_bank` (batch 6); `db:integrity-audit` PASS; tidak ada migrasi pending. Gate teknis/source/MySQL/provider sudah tertutup; hanya manual/browser QA dan shared-cache/multi-worker runtime proof yang ditunda ke Final Phase K. Bukan `COMPLETE + APPROVED`.
9m. K2D Draft Blueprint dari impor (`IMPLEMENTED`; K2D MANUAL / BROWSER QA: DEFERRED BY OWNER; `COMPLETE + APPROVED`: NO): alur `Material → /materials/{material}/blueprints → Manual | Buat dengan AI | Unggah Kisi-kisi DOCX`. Unggah memakai `POST /materials/{material}/blueprint-imports` yang sudah ada, lalu `QuestionBlueprintImport → extraction → interpretation → owner review → explicit grounding → CreateBlueprintDraftFromImport → QuestionBlueprint draft → editor → confirm eksplisit → generation yang sudah ada`. Material Detail menyimpan ringkasan/riwayat dan tautan ke halaman kisi-kisi; form unggah utama tidak ada di sana. Kandidat menjadi baris hanya jika `objective`, `topic`, dan `indicator` berstatus `grounded`. `partial` hanya bila satu-satunya field faktual yang lemah adalah `material`, karena material tidak disimpan sebagai field baris. Kandidat ditolak jika objective/topic/indicator `unresolved`, `ambiguous`, `not_applicable`, kosong, lebih panjang dari batas baris, atau tanpa konteks Profil yang valid. Evidence ambigu tidak dipilih otomatis. Konteks baris diambil dari evidence `objective`/`topic`/`indicator`: kumpulkan `profile_element_id`, deduplikasi, urutkan, validasi lewat `ResolveBlueprintRowContexts`, maksimum 4 konteks unik. Evidence tidak valid atau jumlah unik > 4 menolak kandidat. Tidak ada truncasi diam-diam. Evidence `material` tidak disimpan hanya untuk mengisi konteks. Tidak ada peta alias, konversi otomatis `C1/C2` atau `PG`, atau parser penomoran. `raw_numbering` hanya tampilan/referensi. Owner mengisi `requested_count` dan enum kanonis, serta memilih Simple atau Advanced (Advanced tetap Pro). Seleksi 1–5 kandidat layak. Tidak ada Blueprint overflow. Provenance: `question_blueprint_imports.created_blueprint_id` menunjuk `question_blueprints.blueprint_id` (nullable, tidak unique, `RESTRICT`). Idempotensi: lock baris impor, cek pointer, satu transaksi, update `whereNull`; konversi ulang mengembalikan Blueprint yang ada. Tidak ada UNIQUE index. Otoritas Profil: Blueprint biasa tetap Profil READY terbaru yang cocok. Blueprint terhubung impor memakai `QuestionBlueprintImport.profile_version_id` selama Profil ada, READY, owner dan Material cocok, dan fingerprint cocok. Profil READY yang lebih baru saja tidak membatalkan Blueprint impor. Pin dipakai pada konversi, update draf, confirm, generation start, otoritas child, live authority run, dan rekonstruksi span. Clone tetap menuju Profil READY terbaru dan ditolak sebelum insert jika pin impor berbeda dari Profil terbaru; konteks lama tidak dipetakan ulang. AI fill tidak berubah dan membuat Blueprint terpisah pada jalur Profil terbaru. Unduh DOCX confirmed tidak berubah; label historis boleh tampil dan unduhan tetap jalan. Konversi sinkron, satu transaksi, nol kredit generasi.
