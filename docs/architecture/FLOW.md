...
9h. K2A DOCX Import Foundation: Pemasukan struktur kisi-kisi langsung dari dokumen DOCX. Pondasi ini murni backend: tidak memiliki UI publik, tidak memanggil Gemini, dan tidak menagih kredit. Duplikasi aktif ditolak via `(user_id, material_id, file_hash)`. Proses dieksekusi secara asinkron lewat `ExtractQuestionBlueprintImport` (queue: `material-extraction`), meretry kegagalan operasional, dan segera membuang temporary file saat sukses atau gagal permanen (terminal failure). K2A menyimpan profil Material asal secara utuh untuk validasi keusangan (stale validation) di masa depan.
