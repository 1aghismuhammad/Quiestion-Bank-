...
- Phase 5.7E menambahkan Advanced MCQ: `question_blueprints.mode` plus `ai_fill_requested_total`, Pro aktif via `ResolveActivePro`, mixed difficulty, total 1–30, kualifikasi Run 1–10, `blueprint-fill-v2`, legacy generation default `mcq-v3` (penjelasan tanpa huruf/posisi opsi), confirmed Blueprint Generation Runs use `mcq-v4`, dan presenter shuffle deterministik SHA-256 yang meremap kunci lewat peta kanonis tanpa mengubah `result_json`. Expired-Pro bersifat read-only untuk sumber Advanced existing.
- Phase 5.7F menambahkan True/False dan Essay pada Blueprint dan Generation Run: Simple satu tipe; Advanced mixed type; `ai_fill_requested_type_counts`; `blueprint-fill-v3` untuk fill bertipe historis, `blueprint-fill-v4` (Simple baru), `blueprint-fill-v5` (Advanced baru); `true-false-v1` default legacy (`true-false-v2` untuk Blueprint Run); `essay-v1` default legacy (`essay-v2` untuk Blueprint Run); rekonstruksi dari `question_type` child; shuffle opsi MCQ-only dan ditolak jika tidak ada baris MCQ.
...
- K2A menambahkan pondasi backend untuk import kisi-kisi dari DOCX (pada K2A: tanpa UI publik, tanpa pemanggilan Gemini, dan tanpa konsumsi kredit; unggah owner ada pada K2D). Duplikat aktif ditolak di application level (PENDING/PROCESSING/EXTRACTED) dengan index biasa non-unique `(user_id, material_id, file_hash)`, menyimpan provenance Profil secara utuh, dan menangani kegagalan operasional/terminal secara aman lewat antrean `material-extraction`.
...
- K2B.2 menambahkan job interpretasi Gemini terpisah setelah EXTRACTED. Otoritas input: `structured_document` + `structure_schema_version`. Provider (`QuestionBlueprintImportInterpretationProvider` / `GeminiQuestionBlueprintImportProvider`) mengidentifikasi hubungan semantik lewat source refs saja; server memvalidasi refs dan meresolusi raw field strings dari struktur. Canonical Blueprint fields tidak ditentukan. Status interpretasi (`queued|processing|review_ready|failed`) terpisah dari status ekstraksi. Antrian `material-intelligence` memakai at-least-once delivery + CAS/`interpretation_queued_at` cycle token + `interpretation_claimed_at` lease + `WithoutOverlapping` defense-in-depth; **bukan** `ShouldBeUnique`. Exactly-once HTTP Gemini tidak dijamin; satu hasil persist otoritatif dijamin. Zero credit; no Draft/grounding.
- K2B.3 menambahkan Owner Review Surface read-only. Batas yang diizinkan: menampilkan ringkasan impor terbaru di Material Detail, riwayat impor (15/halaman), tinjauan kandidat raw (20/halaman in-memory), warnings/unresolved, provenance lokasi saja (Option A), polling status minimal, dan retry interpretasi **hanya** saat FAILED. Tidak ada perubahan skema/migrasi/persistensi baru. Nested owner authorization wajib; hasil persist malformed fail-closed. Di luar cakupan K2B.3: grounding Material (K2C), kanonikalisasi/Draft Blueprint (K2D), generasi soal, dan konsumsi kredit. Manual/browser QA ditunda ke Final Phase K (`IMPLEMENTED — SOURCE REVIEW PASS — MANUAL QA DEFERRED`).
- K2C menambahkan Material Grounding: AI memilih `profile_element_ids` semantik terhadap katalog EXTRACTED pinned Profile; server memvalidasi membership/EXTRACTED dan membangun evidence otoritatif (`blueprint-import-grounding-result-v1`). Factual fields only; sparse provider contract; empty/taxonomy shortcut tanpa Gemini. Status `queued|processing|ready|failed` dengan CAS `grounding_queued_at` / `grounding_claimed_at`. Explicit Queue Action saja. Grounding sendiri tidak membuat Draft dan tidak memotong kredit. Status: `IMPLEMENTED — SOURCE REVIEW PASS — MYSQL QA PASS — PROVIDER SMOKE PASS — MANUAL QA DEFERRED` (manual/browser + shared-cache/multi-worker runtime QA tetap Final Phase K; bukan `COMPLETE + APPROVED`). Kontrol owner dan draf impor = K2D.
- K2D menambahkan hub pembuatan kisi-kisi di `/materials/{material}/blueprints`. Tiga jalur: **Buat Manual**, **Buat dengan AI**, dan **Unggah Kisi-kisi DOCX**. Unggah DOCX pindah dari Material Detail ke halaman ini; Material Detail hanya menyimpan ringkasan/riwayat impor dan tautan **Buka Halaman Kisi-kisi**. Setelah unggah, owner meninjau impor, menekan **Cocokkan dengan Materi** (label **Pencocokan**), memilih kandidat yang layak, mengisi isian kanonis, lalu **Simpan sebagai Draf**. Label tampilan lain: Jenis Asesmen, Tingkat Kesulitan, Tipe Soal, Jumlah Soal, Draf, Dikonfirmasi, Menunggu verifikasi, Sedang dicocokkan, Siap, Gagal. Nilai enum tersimpan tidak diubah. Blueprint biasa tetap memakai Profil READY terbaru. Blueprint dari impor memakai Profil yang di-pin impor. Status: `IMPLEMENTED`. K2D MANUAL / BROWSER QA: DEFERRED BY OWNER. `COMPLETE + APPROVED`: NO.
...
Phase 4.3+4.4 implemented Gemini structured MCQ generation and async orchestration on the 4.1+4.2 reservation foundation. Phase 4.5 adds owner-only Blade generation create/show/history/status/retry. Phase 4.6 recovers stale queued/processing reservations. True/false and essay are not provider-executed on the legacy Start path. Phase 5.7F Generation Run children execute typed True/False (`true-false-v1`) and Essay (`essay-v1`) contracts.
...
- K2A menyiapkan pondasi import Blueprint dari DOCX (pada K2A: backend-only, no Gemini, antrian `material-extraction`). K2B.1 menambahkan persistensi struktural import-only. K2B.2 menambahkan interpretasi AI source-ref-only ke `interpretation_result` (`review_ready`). K2B.3 menambahkan Owner Review Surface untuk hasil interpretasi (FAILED retry; manual QA masih ditunda). K2C menambahkan Material Grounding ke `grounding_result` (pinned Profile evidence; explicit queue; zero credit pada grounding). K2D menambahkan hub Manual / AI / Unggah Kisi-kisi DOCX dan konversi tinjauan menjadi draf Blueprint. Unggah publik ada di halaman kisi-kisi, bukan sebagai form unggah utama di Material Detail.
