...
- K2C menambahkan Material Grounding: AI memilih `profile_element_ids` semantik terhadap katalog EXTRACTED pinned Profile; server memvalidasi membership/EXTRACTED dan membangun evidence otoritatif (`blueprint-import-grounding-result-v1`). Factual fields only; sparse provider contract; empty/taxonomy shortcut tanpa Gemini. Status `queued|processing|ready|failed` dengan CAS `grounding_queued_at` / `grounding_claimed_at`. Explicit Queue Action saja. Zero credit / no Draft. Status: `IMPLEMENTED — SOURCE REVIEW PASS — MYSQL QA PASS — PROVIDER SMOKE PASS — MANUAL QA DEFERRED` (manual/browser + shared-cache/multi-worker runtime QA tetap Final Phase K; bukan `COMPLETE + APPROVED`). Draft/public = K2D.
...
- K2A menyiapkan pondasi import Blueprint dari DOCX (backend-only, no UI, no Gemini, menggunakan antrian `material-extraction`). K2B.1 menambahkan persistensi struktural import-only. K2B.2 menambahkan interpretasi AI source-ref-only ke `interpretation_result` (`review_ready`). K2B.3 menambahkan Owner Review Surface read-only (FAILED retry only; no grounding/Draft/generation credit). K2C menambahkan Material Grounding ke `grounding_result` (pinned Profile evidence; explicit queue; zero credit). Draft Blueprint / public upload = K2D.
