...
- K2A menyiapkan pondasi import Blueprint dari DOCX (backend-only, no UI, no Gemini, menggunakan antrian `material-extraction`). K2B.1 menambahkan persistensi struktural import-only. K2B.2 menambahkan interpretasi AI source-ref-only ke `interpretation_result` (`review_ready`). K2B.3 menambahkan Owner Review Surface read-only (FAILED retry only; no grounding/Draft/generation credit). Grounding Material = K2C; Draft Blueprint / public upload = K2D.
