...
- Typed draft edit and publish for MCQ, True/False, and Essay on Run-imported sets. Legacy `generation_id` MCQ import preserved.
...
K2A — Question Blueprint DOCX Import Foundation (`COMPLETE`):

- Backend foundation only for importing Question Blueprints from DOCX files (`question_blueprint_imports` table).
- K2A itself has no public UI, no Gemini calls, no Draft Blueprint import, and no generation. The owner DOCX upload and draft conversion arrived in K2D.
- Enforces duplicate protection via application-level active lookup (PENDING, PROCESSING, EXTRACTED) on `(user_id, material_id, file_hash)`. Supporting indexes are ordinary and non-unique.
...
K2A.1 — Blueprint Import Runtime Hardening (`IMPLEMENTED — QA PENDING`):

- Corrective hardening of the existing K2A import queue/lifecycle. Not a new product feature. No schema, migration, UI, Gemini, Draft Blueprint, or generation credit.
- `ExtractQuestionBlueprintImport` now matches the proven `ExtractMaterialContent` contract: `ShouldBeUnique` (`uniqueId` = `import_id`, `uniqueFor=900`), `WithoutOverlapping` per import (`blueprint-import-extraction:{import_id}`, `releaseAfter=120`, `expireAfter=180`), `tries=3`, `timeout=60` (`<` database `retry_after=90`), `failOnTimeout=true`, `backoff=[10,30,60]`, queue `material-extraction`, `afterCommit`.
- `queued_at` is set when the workflow accepts the enqueue attempt. Dispatch remains outside the User → Material → Profile Version lock transaction.
- Queue dispatch failure after commit terminalizes a still-PENDING import to FAILED (`queue_dispatch_failed`) with a sanitized message. A newer non-PENDING state is not overwritten.
- Source cleanup is best-effort: successful delete nulls `storage_path`; delete failure retains `storage_path` and the terminal business state. FAILED remains re-uploadable.
- PROCESSING remains reclaimable for legitimate retries. EXTRACTED and FAILED remain no-ops.
- Active duplicate rejection stays application-level. Indexes are ordinary and non-unique.
- Status after agent implementation: `IMPLEMENTED — QA PENDING`. Only Council/Owner may promote to `COMPLETE`. Real queue/shared-cache QA remains deferred until final Phase K.

K2B-0 — DOCX Interpretation Input Proof (`PASS`):

- Test-only characterization of current Material `DocxExtractor` `extracted_text` against realistic kisi-kisi table structures.
- Evidence favored Decision B: plain `extracted_text` loses row/cell/matrix/`vMerge`/`gridSpan`/`tblHeader` distinctions (valid structural collisions).
- Material `DocxExtractor` unchanged.

K2B.1 — Structured DOCX Persistence (`IMPLEMENTED — QA PASS`):

- Decision B implemented: import-specific table-aware `structured_document` plus existing `extracted_text`. Schema `blueprint-import-structure-v1`.
- Shared Material `DocxExtractor` remains unchanged and is not a Word table engine.
- Happy path: stored DOCX bytes → import structure extractor → `DocxExtractor` text → persist EXTRACTED with both representations → best-effort source cleanup. Both must succeed before EXTRACTED.
- Additive columns: `structured_document` LONGTEXT NULL, `structure_schema_version` VARCHAR(64) NULL. No index, FK, backfill, or interpretation columns.
- Legacy EXTRACTED rows may have null structural columns. Duplicate protection is not weakened.
- Automated QA: 1370 passed, 7 skipped, 7412 assertions. Real MySQL 8.0.30 migration QA PASS. `db:integrity-audit` PASS.
- Out of scope: Gemini interpretation, candidates, review UI, Draft Blueprint, grounding, generation credit, public upload CTA.

K2B.2 — AI Interpretation Foundation (`COMPLETE + APPROVED`):

- Separate Gemini interpretation job after EXTRACTED. Authoritative input: `structured_document` + `structure_schema_version`. `extracted_text` is not authoritative interpretation input. No Material/Profile grounding.
- Additive nullable columns (migration `2026_09_22_060001`): `interpretation_status`, `interpretation_result`, `interpretation_prompt_version`, `interpretation_error_code`, `interpretation_error_message`, `interpretation_queued_at`, `interpretation_claimed_at`, `interpretation_completed_at`. No index, FK, backfill, or candidate/attempt child table. Historical rows may keep interpretation_* NULL.
- Interpretation statuses: `queued|processing|review_ready|failed`. Extraction and interpretation states are separate. `REVIEW_READY` is terminal for K2B.2. Extraction failure ≠ interpretation failure; interpretation failure does not change EXTRACTED.
- Prompt identity immutable: `blueprint-import-interpret-v1`. Uploaded DOCX content is untrusted DATA. Provider returns source refs only; server validates fail-closed and resolves raw values. Pass-4 `responseSchema` distinguishes paragraph refs (`kind`+`block_ordinal`) from cell refs (`kind`+`block_ordinal`+`table_index`+`row_index`+`cell_index`). No server fallback coordinates.
- Deterministic structure serialization (stable JSON, `JSON_THROW_ON_ERROR`, SHA-256). Application bound 262144 bytes (`input_too_large` at 262145). Candidate cap 100 (101 rejects entire response). Canonical Blueprint fields remain null. Zero generation credit; no `ai_usage_logs`.
- Queue: `database-generation` / `material-intelligence`; `timeout=270`, `tries=3`, `backoff=[5,15]`, `failOnTimeout=false`. **Does not** implement `ShouldBeUnique` (Pass-3 correction). `WithoutOverlapping` `blueprint-import-interpretation:{importId}` (`releaseAfter=60`, `expireAfter=330`). Correctness: at-least-once + CAS cycle token + claim lease + stale 330s + `REVIEW_READY` terminal. Exactly-once Gemini HTTP is not guaranteed; one authoritative persisted result is.
- Automated QA: 1421 passed, 7 skipped, 7685 assertions. Real MySQL 8.0.30 migration/schema QA PASS. `db:integrity-audit` PASS. Real `gemini-3.5-flash-lite` structured-output smoke PASS (Pass-4 anyOf cell coordinates, blueprint_like source-ref resolution, prompt-injection-as-data, taxonomy_non_blueprint → zero candidates, zero-credit / no usage-log side-effect).
...
- Zero credit for grounding itself; grounding does not create a Draft. The owner **Cocokkan dengan Materi** control is part of K2D. Migration `2026_09_23_000001_add_grounding_columns_to_question_blueprint_imports_table` applied on persistent MySQL `ai_question_bank` (batch 6); `db:integrity-audit` PASS; no pending migrations.
...
- Out of scope for K2C itself: Draft Blueprint / public upload (delivered later by K2D), question generation, credit consumption.

K2D — Draft Blueprint creation and public workflow integration (`IMPLEMENTED`).

- Planning: PASS. Combined implementation: PASS. Source review: PASS. Automated QA: PASS. MySQL concurrency QA: PASS. Database QA: PASS. UI creation hub: PASS. UI polish / Indonesian localization: PASS. Staging review: PASS.
...
- Creation hub: `/materials/{material}/blueprints` offers **Buat Manual**, **Buat dengan AI**, and **Unggah Kisi-kisi DOCX**. Material Detail keeps import summary/history and a link to that page. It is not the primary DOCX upload control.
- Flow: Upload DOCX → `QuestionBlueprintImport` → extraction → interpretation → owner review → explicit **Cocokkan dengan Materi** → select eligible candidates → owner fills canonical fields → `CreateBlueprintDraftFromImport` → existing draft editor → explicit confirm → existing generation.
- A candidate becomes a row only when `objective`, `topic`, and `indicator` are `grounded`. `partial` is allowed only when the only weak factual field is `material`. Objective/topic/indicator that is `unresolved`, `ambiguous`, `not_applicable`, empty, over the row limit, or without valid Profile context blocks the candidate. Ambiguous evidence is not auto-selected.
- Contexts come from objective/topic/indicator evidence: collect `profile_element_id`, deduplicate, sort, validate through `ResolveBlueprintRowContexts`, maximum 4 unique contexts. Invalid evidence or more than 4 rejects the candidate. No silent truncation. Material evidence is not stored merely to fill a context.
- No alias map, no automatic `C1/C2` or `PG` conversion, and no numbering parser. `raw_numbering` is display/reference only. The owner enters `requested_count` and canonical enums and chooses Simple or Advanced. Advanced still requires Pro. Selection is 1–5 eligible candidates. No overflow Blueprint.
- Normal Blueprints keep the newest matching READY Profile. An import-linked Blueprint keeps `QuestionBlueprintImport.profile_version_id` while that Profile exists, is READY, owner and Material match, and fingerprints match. A newer READY Profile alone does not invalidate it. The pin applies to conversion, draft update, confirm, generation start, generation child authority, run live authority, and span reconstruction. This is not a global Blueprint Profile change.
- Clone still targets the newest matching READY Profile and is rejected before insert when an import-linked pin differs. Contexts are not remapped. AI fill is unchanged and creates a separate Blueprint on the newest-profile path. Confirmed DOCX download is unchanged; a historical label may show and the download still works.
- One import creates at most one canonical Blueprint through nullable non-unique `created_blueprint_id`, an import row lock, a pointer re-check, one transaction, and a conditional `whereNull` update. Repeated conversion returns the existing Blueprint. There is no UNIQUE index on `created_blueprint_id`.
- Migration `2026_09_23_120000_add_created_blueprint_id_to_question_blueprint_imports_table`: `BIGINT UNSIGNED` NULL, no default, no backfill, FK to `question_blueprints.blueprint_id`, `RESTRICT`, non-unique supporting index (`Non_unique = 1`). Historical imports remained NULL. Database QA PASS.
- Focused coverage: `BlueprintImportDraftConversionTest`, `BlueprintImportOwnerSurfaceTest`, `BlueprintOwnerHttpTest`. Latest full SQLite suite: 1530 passed, 8257 assertions, 0 failures, 0 errors, 9 skipped. The 9 skips are not failures. MySQL `BlueprintImportDraftConversionConcurrencyTest`: 1 passed, 12 assertions, on `ai_question_bank_h3_test` (two workers, same import, one Blueprint, one Series, one pointer, no duplicate canonical draft). Persistent `ai_question_bank` was not the concurrency fixture. No new Gemini smoke.
- Display labels only; stored enums are unchanged: Buat Manual, Buat dengan AI, Unggah Kisi-kisi DOCX, Cocokkan dengan Materi, Pencocokan, Simpan sebagai Draf, Jenis Asesmen, Tingkat Kesulitan, Tipe Soal, Jumlah Soal, Draf, Dikonfirmasi, Menunggu verifikasi, Sedang dicocokkan, Siap, Gagal.
...
2. K2D — Draft Blueprint / public creation hub (`IMPLEMENTED`; K2D MANUAL / BROWSER QA: DEFERRED BY OWNER; `COMPLETE + APPROVED`: NO)
...
- integrated K2 owner workflow after K2C/K2D, including the Blueprint creation hub (K2D MANUAL / BROWSER QA: DEFERRED BY OWNER; not passed)
